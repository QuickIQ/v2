...
import CreativeThinkingUnlockTemplate from './CreativeThinkingUnlockTemplate';
import '../../../App.css';
...
export default function CreativeThinkingUnlockPage() {
  ...
  const { resultLevel } = useCreativeThinkingTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('creative-thinking-test-storage');
        if (saved) {
  ...
  return (
    <CreativeThinkingUnlockTemplate
      level={levelToLoad}
      locale={locale}
    />
  );
