...
import MemoryRetentionUnlockTemplate from './MemoryRetentionUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Hafıza tutma testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your memory retention test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function MemoryRetentionUnlockPage() {
  ...
  const { resultLevel } = useMemoryRetentionTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('memory-retention-test-storage');
        if (saved) {
