...
interface SuccessUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function SuccessUnlockTemplate({ level, locale }: SuccessUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.success.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Başarı Profiliniz' : 'Your Success Profile',
