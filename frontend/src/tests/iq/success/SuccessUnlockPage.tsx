...
import SuccessUnlockTemplate from './SuccessUnlockTemplate';
import '../../../App.css';
...
export default function SuccessUnlockPage() {
  ...
  const { resultLevel } = useSuccessTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('success-test-storage');
        if (saved) {
