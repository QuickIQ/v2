...
import { Lock, Shield, CheckCircle, Star, ArrowDown, Users } from 'lucide-react';
...
function RecentResults({ t, i18n, isMobile }: { t: any; i18n: any; isMobile: boolean }) {
  ...
  return (
    <section style={{
      ...
      >
        {getTranslation('tests.teamPlayer.payment.recent_results', i18n.language === 'tr' ? 'Güncel Sonuçlar' : 'Recent Results')}
      </motion.h2>
      <div style={{
        maxWidth: '800px',
        margin: '0 auto',
...
export default function TeamPlayerPaymentPage() {
  ...
  const { resultLevel, resultData } = useTeamPlayerTestStore();
  ...
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/test/team-player/unlock');
    } catch (error) {
      setProcessing(false);
    ...
    <main style={{
      ...
      <div style={{
        ...
        {/* The Growing Mind Section - Always Visible */}
          <motion.div
            ...
            style={{
              ...
              backdropFilter: 'blur(20px)',
              borderRadius: '32px',
            boxShadow: '0 24px 80px rgba(255, 152, 0, 0.2), 0 0 0 1px rgba(255, 152, 0, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.8)',
        ...
        {resultLevel && resultData ? (
          ...
          >
            ...
            <motion.div 
              ...
              whileHover={{
                scale: 1.02,
                boxShadow: '0 0 40px rgba(255, 152, 0, 0.25)',
                transition: { duration: 0.3 }
              }}
              style={{
                ...
                textAlign: 'center',
                marginTop: '32px',
                boxShadow: '0 4px 25px rgba(255, 152, 0, 0.08)',
                position: 'relative',
                transition: 'all 0.3s ease',
              }}
            >
              ...
              <p style={{
                ...
                {getTranslation('tests.teamPlayer.payment.locked_subtext', 'Your personalized insight is waiting — Discover your hidden strengths, creative growth zones, and unique thinking blueprint.')}
              </p>
              <motion.button
                onClick={() => {
                  const paymentSection = document.getElementById('payment-section');
              ...
              >
                🔓 {getTranslation('tests.teamPlayer.payment.unlock_button', 'Unlock Detailed Result')}
              </motion.button>
            </motion.div>
          </motion.div>
        ) : null}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.teamPlayer.payment.why_trust', i18n.language === 'tr' ? 'Neden QuickIQ\'ya Güvenmelisiniz?' : 'Why Trust QuickIQ?')}
          </h2>
          ...
          <div style={{
            ...
            {[
              {
                icon: <Shield size={32} />,
                title: getTranslation('tests.teamPlayer.payment.trust_feature1_title', i18n.language === 'tr' ? 'Yapay Zeka Destekli Yaratıcılık Analizi' : 'AI-Powered Creativity Analysis'),
                desc: getTranslation('tests.teamPlayer.payment.trust_feature1_desc', i18n.language === 'tr' ? 'Gelişmiş algoritmalarla yaratıcı düşünme tarzınızın derinlemesine analizi.' : 'Deep analysis of your creative thinking style with advanced algorithms.'),
              },
              {
                icon: <CheckCircle size={32} />,
                title: getTranslation('tests.teamPlayer.payment.trust_feature2_title', i18n.language === 'tr' ? 'Uygulanabilir İçgörüler' : 'Actionable Insights'),
                desc: getTranslation('tests.teamPlayer.payment.trust_feature2_desc', i18n.language === 'tr' ? 'Gerçek hayatta kullanabileceğiniz pratik tavsiyeler.' : 'Practical advice you can use in real life.'),
              },
              {
                icon: <Lock size={32} />,
                title: getTranslation('tests.teamPlayer.payment.trust_feature3_title', i18n.language === 'tr' ? 'Kişiselleştirilmiş Gelişim Planı' : 'Personalized Growth Plan'),
                desc: getTranslation('tests.teamPlayer.payment.trust_feature3_desc', i18n.language === 'tr' ? 'Size özel hazırlanmış kapsamlı gelişim stratejisi.' : 'Comprehensive growth strategy tailored just for you.'),
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.teamPlayer.payment.reviews', i18n.language === 'tr' ? 'Yorumlar' : 'Reviews')}
          </h2>
          <p style={{
            ...
            {getTranslation('tests.teamPlayer.payment.reviews_subtitle', i18n.language === 'tr' ? 'Mükemmel ⭐ 4.7 puan — 1769 yorum' : 'Excellent ⭐ 4.7 rating — 1769 reviews')}
          </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{
                      ...
                      {getTranslation('tests.teamPlayer.payment.verified_customer', i18n.language === 'tr' ? 'Doğrulanmış Müşteri' : 'Verified Customer')}
                    </p>
                    <p style={{
                      fontSize: '14px',
                      color: '#666',
      ...
      >
        <p style={{
          ...
          {getTranslation('tests.teamPlayer.payment.footer', i18n.language === 'tr'
        ...
        >
          ...
          <div style={{
            ...
            {['excellent', 'good', 'developing'].map((level) => (
              <Link
                key={level}
                to={`/test/team-player/unlock/${level}`}
                style={{
                  padding: '8px 16px',
