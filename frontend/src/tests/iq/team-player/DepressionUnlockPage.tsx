...
import TeamPlayerUnlockTemplate from './TeamPlayerUnlockTemplate';
import '../../../App.css';
...
export default function TeamPlayerUnlockPage() {
  ...
  const { resultLevel } = useTeamPlayerTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('team-player-test-storage');
        if (saved) {
