...
interface TeamPlayerUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function TeamPlayerUnlockTemplate({ level, locale }: TeamPlayerUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.teamPlayer.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Takım Oyuncusu Profiliniz' : 'Your Team Player Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Team Player Report
          </h1>
          <p style={{
