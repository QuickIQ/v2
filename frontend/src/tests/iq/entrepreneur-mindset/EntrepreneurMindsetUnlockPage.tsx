...
import EntrepreneurMindsetUnlockTemplate from './EntrepreneurMindsetUnlockTemplate';
import '../../../App.css';
...
export default function EntrepreneurMindsetUnlockPage() {
  ...
  const { resultLevel } = useEntrepreneurMindsetTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('entrepreneur-mindset-test-storage');
        if (saved) {
  ...
  return (
    <EntrepreneurMindsetUnlockTemplate
      level={levelToLoad}
      locale={locale}
    />
  );
