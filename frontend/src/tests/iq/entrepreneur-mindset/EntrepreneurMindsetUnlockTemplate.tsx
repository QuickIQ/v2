...
interface EntrepreneurMindsetUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function EntrepreneurMindsetUnlockTemplate({ level, locale }: EntrepreneurMindsetUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.entrepreneurMindset.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Girişimci Zihniyet Profiliniz' : 'Your Entrepreneur Mindset Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Entrepreneur Mindset Report
          </h1>
          <p style={{
