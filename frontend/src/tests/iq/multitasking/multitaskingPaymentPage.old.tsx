...
function RecentResults({ t, i18n, isMobile }: { t: any; i18n: any; isMobile: boolean }) {
  ...
  return (
    <section style={{
      ...
      >
        {getTranslation('tests.multitasking.payment.recent_results', i18n.language === 'tr' ? 'Güncel Sonuçlar' : 'Recent Results')}
      </motion.h2>
      <div style={{
        maxWidth: '800px',
        margin: '0 auto',
...
export default function MultitaskingPaymentPage() {
  ...
  const { resultLevel, resultData } = useMultitaskingTestStore();
  ...
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/test/multitasking/unlock');
    } catch (error) {
      setProcessing(false);
  ...
  const getResultTitle = () => {
    ...
    if (resultLevel === 'excellent') return 'Multitasking Master – The Efficiency Expert';
    if (resultLevel === 'good') return 'Multitasking Pro – The Balanced Coordinator';
    return 'Multitasking Learner – The Developing Manager';
  };
  ...
  const getPlaceholderInsights = () => [
    'Task Management Patterns: Discover how you handle multiple responsibilities',
    'Focus Efficiency: Learn to recognize and optimize your attention distribution',
    'Prioritization Skills: Explore your ability to manage competing tasks',
    'Productivity Strategies: Practical steps to improve your multitasking efficiency',
    'Future Potential: Unlock your path to enhanced task coordination and balance',
    'Workflow Essentials: Build daily habits that support your multitasking capabilities'
  ];
...
Your Multitasking Efficiency Journey
              </h2>
          ...
          >
            ...
            >
              ...
              <p style={{
                ...
                {getTranslation('tests.multitasking.payment.locked_subtext', 'Your personalized insight is waiting — Discover your hidden strengths, creative growth zones, and unique thinking blueprint.')}
              </p>
              <motion.button
                onClick={() => {
                  const paymentSection = document.getElementById('payment-section');
              ...
              >
                🔓 {getTranslation('tests.multitasking.payment.unlock_button', 'Unlock Detailed Result')}
              </motion.button>
            </motion.div>
          </motion.div>
        ) : null}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.multitasking.payment.why_trust', i18n.language === 'tr' ? 'Neden QuickIQ\'ya Güvenmelisiniz?' : 'Why Trust QuickIQ?')}
          </h2>
          ...
          <div style={{
            ...
            {[
              {
                icon: <Shield size={32} />,
                title: getTranslation('tests.multitasking.payment.trust_feature1_title', i18n.language === 'tr' ? 'Yapay Zeka Destekli Yaratıcılık Analizi' : 'AI-Powered Creativity Analysis'),
                desc: getTranslation('tests.multitasking.payment.trust_feature1_desc', i18n.language === 'tr' ? 'Gelişmiş algoritmalarla yaratıcı düşünme tarzınızın derinlemesine analizi.' : 'Deep analysis of your creative thinking style with advanced algorithms.'),
              },
              {
                icon: <CheckCircle size={32} />,
                title: getTranslation('tests.multitasking.payment.trust_feature2_title', i18n.language === 'tr' ? 'Uygulanabilir İçgörüler' : 'Actionable Insights'),
                desc: getTranslation('tests.multitasking.payment.trust_feature2_desc', i18n.language === 'tr' ? 'Gerçek hayatta kullanabileceğiniz pratik tavsiyeler.' : 'Practical advice you can use in real life.'),
              },
              {
                icon: <Lock size={32} />,
                title: getTranslation('tests.multitasking.payment.trust_feature3_title', i18n.language === 'tr' ? 'Kişiselleştirilmiş Gelişim Planı' : 'Personalized Growth Plan'),
                desc: getTranslation('tests.multitasking.payment.trust_feature3_desc', i18n.language === 'tr' ? 'Size özel hazırlanmış kapsamlı gelişim stratejisi.' : 'Comprehensive growth strategy tailored just for you.'),
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.multitasking.payment.reviews', i18n.language === 'tr' ? 'Yorumlar' : 'Reviews')}
          </h2>
          <p style={{
            ...
            {getTranslation('tests.multitasking.payment.reviews_subtitle', i18n.language === 'tr' ? 'Mükemmel ⭐ 4.7 puan — 1769 yorum' : 'Excellent ⭐ 4.7 rating — 1769 reviews')}
          </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{
                      ...
                      {getTranslation('tests.multitasking.payment.verified_customer', i18n.language === 'tr' ? 'Doğrulanmış Müşteri' : 'Verified Customer')}
                    </p>
                    <p style={{
                      fontSize: '14px',
                      color: '#666',
      ...
      >
        <p style={{
          ...
          {getTranslation('tests.multitasking.payment.footer', i18n.language === 'tr'
