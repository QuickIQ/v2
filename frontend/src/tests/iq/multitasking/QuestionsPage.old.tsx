...
import { useMultitaskingTestStore, MultitaskingQuestion, MultitaskingAnswer } from '../../../store/multitaskingTestStore';
import { useMobile } from '../../../hooks/useMobile';
import { Clock } from 'lucide-react';
import '../../../App.css';

interface Props {
  questions: MultitaskingQuestion[];
  onComplete: () => void;
}
...
function MultitaskingQuestionsPage({ questions, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    let currentTime = useMultitaskingTestStore.getState().timeRemaining;
    ...
    timerIntervalRef.current = setInterval(() => {
      const storeTime = useMultitaskingTestStore.getState().timeRemaining;
  ...
  const handleOptionSelect = (optionIndex: number) => {
    ...
    const answer: MultitaskingAnswer = {
      ...
      <div style={{
        ...
        <div className="card" style={{
          ...
          <div style={{ color: '#e74c3c', marginBottom: '20px' }}>
            {t('tests.multitasking.errors.load_questions') || 'Failed to load questions. Please try again.'}
          </div>
        </div>
      </div>
    );
...
export default MultitaskingQuestionsPage;
