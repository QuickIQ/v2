...
import MultitaskingUnlockTemplate from './MultitaskingUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Çoklu görev verimliliği testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your multitasking efficiency test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function MultitaskingUnlockPage() {
  ...
  const { resultLevel } = useMultitaskingTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('multitasking-test-storage');
        if (saved) {
