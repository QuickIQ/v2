...
import AmbitionUnlockTemplate from './AmbitionUnlockTemplate';
import '../../../App.css';
...
export default function AmbitionUnlockPage() {
  ...
  const { resultLevel } = useAmbitionTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('ambition-test-storage');
        if (saved) {
