...
interface AmbitionUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function AmbitionUnlockTemplate({ level, locale }: AmbitionUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.ambition.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Hırs Profiliniz' : 'Your Ambition Profile',
