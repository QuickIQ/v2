...
import DecisionMakingUnlockTemplate from './DecisionMakingUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Karar verme testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your decision making test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function DecisionMakingUnlockPage() {
  ...
  const { resultLevel } = useDecisionMakingTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('decision-making-test-storage');
        if (saved) {
