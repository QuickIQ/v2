...
interface DecisionMakingUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function DecisionMakingUnlockTemplate({ level, locale }: DecisionMakingUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.decisionMaking.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Karar Verme Profiliniz' : 'Your Decision Making Profile',
