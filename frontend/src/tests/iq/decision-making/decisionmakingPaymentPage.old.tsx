...
function RecentResults({ t, i18n, isMobile }: { t: any; i18n: any; isMobile: boolean }) {
  ...
  return (
    <section style={{
      ...
      >
        {getTranslation('tests.decisionMaking.payment.recent_results', i18n.language === 'tr' ? 'Güncel Sonuçlar' : 'Recent Results')}
      </motion.h2>
      <div style={{
        maxWidth: '800px',
        margin: '0 auto',
...
export default function DecisionMakingPaymentPage() {
  ...
  const { resultLevel, resultData } = useDecisionMakingTestStore();
  ...
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/test/decision-making/unlock');
    } catch (error) {
      setProcessing(false);
  ...
  const getResultEmojis = () => {
    if (resultLevel === 'excellent') return ['⚖️', '✨', '🎯'];
    ...
    <main style={{
      ...
      <div style={{
          ...
          >
            ...
            <div style={{
              ...
              <h2 style={{
                ...
                fontWeight: '700',
                background: 'linear-gradient(135deg, #6c63ff 0%, #9bc9ed 50%, #6c63ff 100%)',
                ...
                Your Detailed Decision Making Result
              </h2>
          ...
          >
            ...
            <motion.div 
              ...
              style={{
                ...
                background: 'linear-gradient(180deg, #E8F0FF 0%, #F0E8FF 100%)',
                borderRadius: '22px',
            ...
            >
              ...
              <p style={{
                ...
                {getTranslation('tests.decisionMaking.payment.locked_subtext', 'Your personalized insight is waiting — Discover your hidden strengths, creative growth zones, and unique thinking blueprint.')}
              </p>
              <motion.button
                onClick={() => {
                  const paymentSection = document.getElementById('payment-section');
              ...
              >
                🔓 {getTranslation('tests.decisionMaking.payment.unlock_button', 'Unlock Detailed Result')}
              </motion.button>
            </motion.div>
          </motion.div>
        ) : null}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.decisionMaking.payment.why_trust', i18n.language === 'tr' ? 'Neden QuickIQ\'ya Güvenmelisiniz?' : 'Why Trust QuickIQ?')}
          </h2>
          ...
          <div style={{
            ...
            {[
              {
                icon: <Shield size={32} />,
                title: getTranslation('tests.decisionMaking.payment.trust_feature1_title', i18n.language === 'tr' ? 'Yapay Zeka Destekli Yaratıcılık Analizi' : 'AI-Powered Creativity Analysis'),
                desc: getTranslation('tests.decisionMaking.payment.trust_feature1_desc', i18n.language === 'tr' ? 'Gelişmiş algoritmalarla yaratıcı düşünme tarzınızın derinlemesine analizi.' : 'Deep analysis of your creative thinking style with advanced algorithms.'),
              },
              {
                icon: <CheckCircle size={32} />,
                title: getTranslation('tests.decisionMaking.payment.trust_feature2_title', i18n.language === 'tr' ? 'Uygulanabilir İçgörüler' : 'Actionable Insights'),
                desc: getTranslation('tests.decisionMaking.payment.trust_feature2_desc', i18n.language === 'tr' ? 'Gerçek hayatta kullanabileceğiniz pratik tavsiyeler.' : 'Practical advice you can use in real life.'),
              },
              {
                icon: <Lock size={32} />,
                title: getTranslation('tests.decisionMaking.payment.trust_feature3_title', i18n.language === 'tr' ? 'Kişiselleştirilmiş Gelişim Planı' : 'Personalized Growth Plan'),
                desc: getTranslation('tests.decisionMaking.payment.trust_feature3_desc', i18n.language === 'tr' ? 'Size özel hazırlanmış kapsamlı gelişim stratejisi.' : 'Comprehensive growth strategy tailored just for you.'),
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.decisionMaking.payment.reviews', i18n.language === 'tr' ? 'Yorumlar' : 'Reviews')}
          </h2>
          <p style={{
            ...
            {getTranslation('tests.decisionMaking.payment.reviews_subtitle', i18n.language === 'tr' ? 'Mükemmel ⭐ 4.7 puan — 1769 yorum' : 'Excellent ⭐ 4.7 rating — 1769 reviews')}
          </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{
                      ...
                      {getTranslation('tests.decisionMaking.payment.verified_customer', i18n.language === 'tr' ? 'Doğrulanmış Müşteri' : 'Verified Customer')}
                    </p>
                    <p style={{
                      fontSize: '14px',
                      color: '#666',
        ...
        >
          ...
          <div style={{
            ...
            {['excellent', 'good', 'developing'].map((level) => (
              <Link
                key={level}
                to={`/test/decision-making/unlock/${level}`}
                style={{
                  padding: '8px 16px',
      ...
      >
        <p style={{
          ...
          {getTranslation('tests.decisionMaking.payment.footer', i18n.language === 'tr'
