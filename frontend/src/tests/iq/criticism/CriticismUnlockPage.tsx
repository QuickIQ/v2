...
import CriticismUnlockTemplate from './CriticismUnlockTemplate';
import '../../../App.css';
...
export default function CriticismUnlockPage() {
  ...
  const { resultLevel } = useCriticismTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('criticism-test-storage');
        if (saved) {
