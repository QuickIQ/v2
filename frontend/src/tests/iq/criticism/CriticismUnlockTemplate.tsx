...
interface CriticismUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function CriticismUnlockTemplate({ level, locale }: CriticismUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.criticism.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Eleştiri Profiliniz' : 'Your Criticism Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Criticism Report
          </h1>
          <p style={{
