...
export default function AutismUnlockTemplate({ level, locale }: AutismUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.autism.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Otizm Spektrum Profiliniz' : 'Your Autism Spectrum Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Autism Spectrum Report
          </h1>
          <p style={{
