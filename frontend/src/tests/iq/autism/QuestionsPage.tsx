...
import { useAutismTestStore, AutismQuestion, AutismAnswer } from '../../../store/autismTestStore';
import { useMobile } from '../../../hooks/useMobile';
import { Clock } from 'lucide-react';
import '../../../App.css';

interface Props {
  questions: AutismQuestion[];
  onComplete: () => void;
}

const optionGradient = 'linear-gradient(135deg, #6c63ff 0%, #9bc9ed 100%)';

function AutismQuestionsPage({ questions, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    let currentTime = useAutismTestStore.getState().timeRemaining;
    ...
    timerIntervalRef.current = setInterval(() => {
      const storeTime = useAutismTestStore.getState().timeRemaining;
      ...
      <div style={{
        ...
        <div className="card" style={{
          ...
          <div style={{ color: '#e74c3c', marginBottom: '20px' }}>
            {t('tests.autism.errors.load_questions') || 'Failed to load questions. Please try again.'}
          </div>
        </div>
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {/* Header: Autism Test + Question Progress + Timer - Just above the card */}
            <motion.div
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              style={{
            ...
            >
              <div style={{ 
                ...
                {t('tests.autism.questions.testName', 'Autism Test')}
              </div>
