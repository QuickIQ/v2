...
function RecentResults({ t, i18n, isMobile }: { t: any; i18n: any; isMobile: boolean }) {
  ...
  return (
    <section style={{
      ...
      >
        {getTranslation('tests.autism.payment.recent_results', i18n.language === 'tr' ? 'Güncel Sonuçlar' : 'Recent Results')}
      </motion.h2>
      <div style={{
        maxWidth: '800px',
        margin: '0 auto',
...
export default function AutismPaymentPage() {
  ...
  const { resultLevel, resultData } = useAutismTestStore();
  ...
  const [testCount, setTestCount] = useState(() => {
    ...
    const saved = localStorage.getItem('autismTestsCount');
    const savedTime = localStorage.getItem('autismTestsTime');
    const now = Date.now();
    ...
    localStorage.setItem('autismTestsCount', newCount.toString());
    localStorage.setItem('autismTestsTime', now.toString());
    return newCount;
  });
    ...
    const checkReset = setInterval(() => {
      ...
      if (savedTime) {
        ...
        if (timeDiff >= 86400000) {
          ...
          localStorage.setItem('autismTestsCount', newCount.toString());
          localStorage.setItem('autismTestsTime', Date.now().toString());
        }
      }
    }, 3600000);
  ...
  const reviews = {
    en: [
      ...
      { name: 'Sophia T.', text: 'Amazing accuracy! Everything it said about my autism was true. Great investment in self-discovery.', city: 'Berlin, Germany', time: '1 day ago' },
    ...
    <main style={{
      ...
      <div style={{
        ...
        {resultLevel && resultData ? (
          ...
          >
            ...
            <p style={{
              ...
              {getTranslation('tests.autism.payment.your_result', 'Your Creativity Level')}
            </p>
            ...
            <p style={{
              ...
              {getTranslation('tests.autism.payment.blueprint_text', 'Your autism analysis has been generated.')}
            </p>
            ...
            >
              ...
              <p style={{
                ...
                {getTranslation('tests.autism.payment.locked_subtext', 'Your full detailed report is ready — including creative strategies, growth areas, and personalized insights.')}
              </p>
              <motion.button
                onClick={() => {
                  const paymentSection = document.getElementById('payment-section');
              ...
              >
                🔓 {getTranslation('tests.autism.payment.unlock_button', 'Unlock Detailed Result')}
              </motion.button>
            </motion.div>
            </div>
          </motion.div>
        ...
        >
          ...
          >
            {getTranslation('tests.autism.payment.tests_completed', i18n.language === 'tr' ? 'Bugün' : 'Today')} <span style={{ fontWeight: '700', color: '#6c63ff' }}>{testCount.toLocaleString()}</span> {getTranslation('tests.autism.payment.tests_completed_suffix', i18n.language === 'tr' ? 'kişi testi tamamladı' : 'people completed the test')}
          </motion.p>
        </motion.div>
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.autism.payment.why_trust', i18n.language === 'tr' ? 'Neden QuickIQ\'ya Güvenmelisiniz?' : 'Why Trust QuickIQ?')}
          </h2>
          ...
          <div style={{
            ...
            {[
              {
                icon: <Shield size={32} />,
                title: getTranslation('tests.autism.payment.trust_feature1_title', i18n.language === 'tr' ? 'Yapay Zeka Destekli Yaratıcılık Analizi' : 'AI-Powered Creativity Analysis'),
                desc: getTranslation('tests.autism.payment.trust_feature1_desc', i18n.language === 'tr' ? 'Gelişmiş algoritmalarla yaratıcı düşünme tarzınızın derinlemesine analizi.' : 'Deep analysis of your autism style with advanced algorithms.'),
              },
              {
                icon: <CheckCircle size={32} />,
                title: getTranslation('tests.autism.payment.trust_feature2_title', i18n.language === 'tr' ? 'Uygulanabilir İçgörüler' : 'Actionable Insights'),
                desc: getTranslation('tests.autism.payment.trust_feature2_desc', i18n.language === 'tr' ? 'Gerçek hayatta kullanabileceğiniz pratik tavsiyeler.' : 'Practical advice you can use in real life.'),
              },
              {
                icon: <Lock size={32} />,
                title: getTranslation('tests.autism.payment.trust_feature3_title', i18n.language === 'tr' ? 'Kişiselleştirilmiş Gelişim Planı' : 'Personalized Growth Plan'),
                desc: getTranslation('tests.autism.payment.trust_feature3_desc', i18n.language === 'tr' ? 'Size özel hazırlanmış kapsamlı gelişim stratejisi.' : 'Comprehensive growth strategy tailored just for you.'),
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.autism.payment.reviews', i18n.language === 'tr' ? 'Yorumlar' : 'Reviews')}
          </h2>
          <p style={{
            fontSize: isMobile ? '16px' : '18px',
            color: '#666',
            ...
            {getTranslation('tests.autism.payment.reviews_subtitle', i18n.language === 'tr' ? 'Mükemmel ⭐ 4.7 puan — 1769 yorum' : 'Excellent ⭐ 4.7 rating — 1769 reviews')}
          </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{
                      ...
                      {getTranslation('tests.autism.payment.verified_customer', i18n.language === 'tr' ? 'Doğrulanmış Müşteri' : 'Verified Customer')}
                    </p>
                    <p style={{
                      fontSize: '14px',
                      color: '#666',
        ...
        >
          <p style={{
            ...
            {getTranslation('tests.autism.payment.footer', i18n.language === 'tr'
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <p style={{
                ...
                / 7-Day Trial {getTranslation('tests.autism.payment.trial_footer', i18n.language === 'tr' 
