...
import TimeManagementUnlockTemplate from './TimeManagementUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Zaman yönetimi testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your time management test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function TimeManagementUnlockPage() {
  ...
  const { resultLevel } = useTimeManagementTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('time-management-test-storage');
        if (saved) {
