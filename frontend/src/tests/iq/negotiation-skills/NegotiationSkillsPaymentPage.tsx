...
import { Lock, Shield, CheckCircle, Star, ArrowDown, UsersRound } from 'lucide-react';
...
function RecentResults({ t, i18n, isMobile }: { t: any; i18n: any; isMobile: boolean }) {
  ...
  return (
    <section style={{
      ...
      >
        {getTranslation('tests.negotiationSkills.payment.recent_results', i18n.language === 'tr' ? 'Güncel Sonuçlar' : 'Recent Results')}
      </motion.h2>
      <div style={{
        maxWidth: '800px',
        margin: '0 auto',
...
export default function NegotiationSkillsPaymentPage() {
  ...
  const { resultLevel, resultData } = useNegotiationSkillsTestStore();
  ...
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/test/negotiation-skills/unlock');
    } catch (error) {
      setProcessing(false);
    ...
    <main style={{
      ...
      <div style={{
          ...
          >
            ...
            <div style={{
              ...
              <h2 style={{
                ...
                Your Detailed Negotiation Skills Result
              </h2>
          ...
          >
            ...
            >
              ...
              <p style={{
                ...
                {getTranslation('tests.negotiationSkills.payment.locked_subtext', 'Your personalized insight is waiting — Discover your hidden strengths, creative growth zones, and unique thinking blueprint.')}
              </p>
              <motion.button
                onClick={() => {
                  const paymentSection = document.getElementById('payment-section');
              ...
              >
                🔓 {getTranslation('tests.negotiationSkills.payment.unlock_button', 'Unlock Detailed Result')}
              </motion.button>
            </motion.div>
          </motion.div>
        ) : null}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.negotiationSkills.payment.why_trust', i18n.language === 'tr' ? 'Neden QuickIQ\'ya Güvenmelisiniz?' : 'Why Trust QuickIQ?')}
          </h2>
          ...
          <div style={{
            ...
            {[
              {
                icon: <Shield size={32} />,
                title: getTranslation('tests.negotiationSkills.payment.trust_feature1_title', i18n.language === 'tr' ? 'Yapay Zeka Destekli Yaratıcılık Analizi' : 'AI-Powered Creativity Analysis'),
                desc: getTranslation('tests.negotiationSkills.payment.trust_feature1_desc', i18n.language === 'tr' ? 'Gelişmiş algoritmalarla yaratıcı düşünme tarzınızın derinlemesine analizi.' : 'Deep analysis of your creative thinking style with advanced algorithms.'),
              },
              {
                icon: <CheckCircle size={32} />,
                title: getTranslation('tests.negotiationSkills.payment.trust_feature2_title', i18n.language === 'tr' ? 'Uygulanabilir İçgörüler' : 'Actionable Insights'),
                desc: getTranslation('tests.negotiationSkills.payment.trust_feature2_desc', i18n.language === 'tr' ? 'Gerçek hayatta kullanabileceğiniz pratik tavsiyeler.' : 'Practical advice you can use in real life.'),
              },
              {
                icon: <Lock size={32} />,
                title: getTranslation('tests.negotiationSkills.payment.trust_feature3_title', i18n.language === 'tr' ? 'Kişiselleştirilmiş Gelişim Planı' : 'Personalized Growth Plan'),
                desc: getTranslation('tests.negotiationSkills.payment.trust_feature3_desc', i18n.language === 'tr' ? 'Size özel hazırlanmış kapsamlı gelişim stratejisi.' : 'Comprehensive growth strategy tailored just for you.'),
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <h2 style={{
            ...
            {getTranslation('tests.negotiationSkills.payment.reviews', i18n.language === 'tr' ? 'Yorumlar' : 'Reviews')}
          </h2>
          <p style={{
            ...
            {getTranslation('tests.negotiationSkills.payment.reviews_subtitle', i18n.language === 'tr' ? 'Mükemmel ⭐ 4.7 puan — 1769 yorum' : 'Excellent ⭐ 4.7 rating — 1769 reviews')}
          </p>
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{
                      ...
                      {getTranslation('tests.negotiationSkills.payment.verified_customer', i18n.language === 'tr' ? 'Doğrulanmış Müşteri' : 'Verified Customer')}
                    </p>
                    <p style={{
                      fontSize: '14px',
                      color: '#666',
      ...
      >
        <p style={{
          ...
          {getTranslation('tests.negotiationSkills.payment.footer', i18n.language === 'tr'
        ...
        >
          ...
          <div style={{
            ...
            {['excellent', 'good', 'developing'].map((level) => (
              <Link
                key={level}
                to={`/test/negotiation-skills/unlock/${level}`}
                style={{
                  padding: '8px 16px',
