...
import NegotiationSkillsUnlockTemplate from './NegotiationSkillsUnlockTemplate';
import '../../../App.css';
...
export default function NegotiationSkillsUnlockPage() {
  ...
  const { resultLevel } = useNegotiationSkillsTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('negotiation-skills-test-storage');
        if (saved) {
  ...
  return (
    <NegotiationSkillsUnlockTemplate
      level={levelToLoad}
      locale={locale}
    />
  );
