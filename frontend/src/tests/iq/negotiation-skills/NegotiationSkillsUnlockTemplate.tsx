...
interface NegotiationSkillsUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function NegotiationSkillsUnlockTemplate({ level, locale }: NegotiationSkillsUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.negotiationSkills.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Müzakere Becerileriniz Profili' : 'Your Negotiation Skills Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Negotiation Skills Report
          </h1>
          <p style={{
