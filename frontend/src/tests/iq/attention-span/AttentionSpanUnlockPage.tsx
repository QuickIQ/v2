...
import AttentionSpanUnlockTemplate from './AttentionSpanUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Odaklanma süresi testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your attention span test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function AttentionSpanUnlockPage() {
  ...
  const { resultLevel } = useAttentionSpanTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('attention-span-test-storage');
        if (saved) {
