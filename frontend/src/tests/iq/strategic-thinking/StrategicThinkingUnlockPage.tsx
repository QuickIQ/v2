...
import StrategicThinkingUnlockTemplate from './StrategicThinkingUnlockTemplate';
import '../../../App.css';
...
export default function StrategicThinkingUnlockPage() {
  ...
  const { resultLevel } = useStrategicThinkingTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('strategic-thinking-test-storage');
        if (saved) {
  ...
  return (
    <StrategicThinkingUnlockTemplate
      level={levelToLoad}
      locale={locale}
    />
  );
