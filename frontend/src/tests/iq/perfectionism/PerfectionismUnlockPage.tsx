...
import PerfectionismUnlockTemplate from './PerfectionismUnlockTemplate';
import '../../../App.css';
...
export default function PerfectionismUnlockPage() {
  ...
  const { resultLevel } = usePerfectionismTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('perfectionism-test-storage');
        if (saved) {
