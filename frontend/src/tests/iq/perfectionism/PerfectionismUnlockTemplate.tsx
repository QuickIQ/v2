...
interface PerfectionismUnlockTemplateProps {
  level: 'excellent' | 'good' | 'developing';
  locale?: 'en' | 'tr';
}

export default function PerfectionismUnlockTemplate({ level, locale }: PerfectionismUnlockTemplateProps) {
  ...
  const getContent = () => {
    const baseKey = `tests.perfectionism.result.${level}`;
    return {
  ...
  const sections = [
    {
      ...
      title: currentLocale === 'tr' ? 'Mükemmeliyetçilik Profiliniz' : 'Your Perfectionism Profile',
      ...
      <div style={{
        ...
        >
          ...
          <h1 style={{
            ...
            Your Detailed Perfectionism Report
          </h1>
          <p style={{
