...
import { usePerfectionismTestStore, PerfectionismQuestion, PerfectionismAnswer } from '../../../store/perfectionismTestStore';
import { useMobile } from '../../../hooks/useMobile';
import { Clock, Target } from 'lucide-react';
import '../../../App.css';

interface Props {
  questions: PerfectionismQuestion[];
  onComplete: () => void;
}
...
function PerfectionismQuestionsPage({ questions, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    let currentTime = usePerfectionismTestStore.getState().timeRemaining;
    ...
    timerIntervalRef.current = setInterval(() => {
      const storeTime = usePerfectionismTestStore.getState().timeRemaining;
  ...
  const handleOptionSelect = (optionIndex: number) => {
    ...
    const answer: PerfectionismAnswer = {
      ...
      <div style={{
        ...
        <div className="card" style={{
          ...
          <div style={{ color: '#e74c3c', marginBottom: '20px' }}>
            {t('tests.perfectionism.errors.load_questions') || 'Failed to load questions. Please try again.'}
          </div>
        </div>
      </div>
    );
...
export default PerfectionismQuestionsPage;
