...
import StressManagementUnlockTemplate from './StressManagementUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Stres yönetimi testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your stress management test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function StressManagementUnlockPage() {
  ...
  const { resultLevel } = useStressManagementTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('stress-management-test-storage');
        if (saved) {
  ...
  return (
    <StressManagementUnlockTemplate
      level={levelToLoad}
      locale={locale}
    />
  );
