...
import RiskToleranceUnlockTemplate from './RiskToleranceUnlockTemplate';
import '../../../App.css';
    ...
    <main style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {i18n.language === 'tr'
            ? 'Risk toleransı testi sonuçlarınız bulunamadı. Lütfen testi tekrar tamamlayın.'
            : 'Your risk tolerance test results were not found. Please complete the test again.'}
        </p>
      </div>
    </main>
  );
...
export default function RiskToleranceUnlockPage() {
  ...
  const { resultLevel } = useRiskToleranceTestStore();
  ...
  useEffect(() => {
    ...
    if (!level) {
      try {
        const saved = localStorage.getItem('risk-tolerance-test-storage');
        if (saved) {
