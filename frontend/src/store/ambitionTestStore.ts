...
export interface AmbitionQuestion {
  id: number;
  text: string;
...
export interface AmbitionAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface AmbitionTestState {
  // Session
  ...
  setQuestions: (questions: AmbitionQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: AmbitionAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: AmbitionTestState['step']) => void;
  reset: () => void;
}
...
export const useAmbitionTestStore = create<AmbitionTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: AmbitionQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: AmbitionAnswer) => {
      ...
      setStep: (step: AmbitionTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'ambition-test-storage',
      partialize: (state) => ({
