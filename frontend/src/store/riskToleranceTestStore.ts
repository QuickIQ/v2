...
export interface RiskToleranceTestState {
  // Session
  ...
  setQuestions: (questions: RiskToleranceQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: RiskToleranceAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: RiskToleranceTestState['step']) => void;
  reset: () => void;
}
...
export const useRiskToleranceTestStore = create<RiskToleranceTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: RiskToleranceQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: RiskToleranceAnswer) => {
      ...
      setStep: (step: RiskToleranceTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'risk-tolerance-test-storage',
      partialize: (state) => ({
