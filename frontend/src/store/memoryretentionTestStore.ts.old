...
export interface MemoryRetentionQuestion {
  id: number;
  text: string;
...
export interface MemoryRetentionAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface MemoryRetentionTestState {
  // Session
  ...
  setQuestions: (questions: MemoryRetentionQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: MemoryRetentionAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: MemoryRetentionTestState['step']) => void;
  reset: () => void;
}
...
export const useMemoryRetentionTestStore = create<MemoryRetentionTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: MemoryRetentionQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: MemoryRetentionAnswer) => {
      ...
      setStep: (step: MemoryRetentionTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'memory-retention-test-storage',
      partialize: (state) => ({
