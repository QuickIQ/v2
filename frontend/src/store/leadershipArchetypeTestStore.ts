...
export interface LeadershipArchetypeQuestion {
  id: number;
  text: string;
...
export interface LeadershipArchetypeAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface LeadershipArchetypeTestState {
  // Session
  ...
  answers: LeadershipArchetypeAnswer[];
  ...
  setQuestions: (questions: LeadershipArchetypeQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: LeadershipArchetypeAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: LeadershipArchetypeTestState['step']) => void;
  reset: () => void;
}
...
export const useLeadershipArchetypeTestStore = create<LeadershipArchetypeTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: LeadershipArchetypeQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: LeadershipArchetypeAnswer) => {
      ...
      setStep: (step: LeadershipArchetypeTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'leadership-archetype-test-storage',
      partialize: (state) => ({
