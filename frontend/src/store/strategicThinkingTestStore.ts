...
export interface StrategicThinkingQuestion {
  id: number;
  text: string;
...
export interface StrategicThinkingAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface StrategicThinkingTestState {
  // Session
  ...
  answers: StrategicThinkingAnswer[];
  ...
  setQuestions: (questions: StrategicThinkingQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: StrategicThinkingAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: StrategicThinkingTestState['step']) => void;
  reset: () => void;
}
...
export const useStrategicThinkingTestStore = create<StrategicThinkingTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: StrategicThinkingQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: StrategicThinkingAnswer) => {
      ...
      setStep: (step: StrategicThinkingTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'strategic-thinking-test-storage',
      partialize: (state) => ({
