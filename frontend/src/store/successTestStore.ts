...
export interface SuccessTestState {
  // Session
  ...
  setQuestions: (questions: SuccessQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: SuccessAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: SuccessTestState['step']) => void;
  reset: () => void;
}
...
export const useSuccessTestStore = create<SuccessTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: SuccessQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: SuccessAnswer) => {
      ...
      setStep: (step: SuccessTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'success-test-storage',
      partialize: (state) => ({
