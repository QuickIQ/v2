...
export interface PerfectionismAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface PerfectionismTestState {
  // Session
  ...
  setQuestions: (questions: PerfectionismQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: PerfectionismAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: PerfectionismTestState['step']) => void;
  reset: () => void;
}
...
export const usePerfectionismTestStore = create<PerfectionismTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: PerfectionismQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: PerfectionismAnswer) => {
      ...
      setStep: (step: PerfectionismTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'perfectionism-test-storage',
      partialize: (state) => ({
