...
export interface DecisionMakingAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface DecisionMakingTestState {
  // Session
  ...
  setQuestions: (questions: DecisionMakingQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: DecisionMakingAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: DecisionMakingTestState['step']) => void;
  reset: () => void;
}
...
export const useDecisionMakingTestStore = create<DecisionMakingTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: DecisionMakingQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: DecisionMakingAnswer) => {
      ...
      setStep: (step: DecisionMakingTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'decision-making-test-storage',
      partialize: (state) => ({
