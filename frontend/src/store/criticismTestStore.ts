...
export interface CriticismQuestion {
  id: number;
  text: string;
...
export interface CriticismAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface CriticismTestState {
  // Session
  ...
  setQuestions: (questions: CriticismQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: CriticismAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: CriticismTestState['step']) => void;
  reset: () => void;
}
...
export const useCriticismTestStore = create<CriticismTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: CriticismQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: CriticismAnswer) => {
      ...
      setStep: (step: CriticismTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'criticism-test-storage',
      partialize: (state) => ({
