...
export interface StressManagementQuestion {
  id: number;
  text: string;
...
export interface StressManagementAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface StressManagementTestState {
  // Session
  ...
  // Test data
  questions: StressManagementQuestion[];
  answers: StressManagementAnswer[];
  ...
  setQuestions: (questions: StressManagementQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: StressManagementAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: StressManagementTestState['step']) => void;
  reset: () => void;
}
...
export const useStressManagementTestStore = create<StressManagementTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: StressManagementQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: StressManagementAnswer) => {
      ...
      setStep: (step: StressManagementTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'stress-management-test-storage',
      partialize: (state) => ({
