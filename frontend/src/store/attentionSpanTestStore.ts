...
export interface AttentionSpanQuestion {
  id: number;
  text: string;
...
export interface AttentionSpanAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface AttentionSpanTestState {
  // Session
  ...
  setQuestions: (questions: AttentionSpanQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: AttentionSpanAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: AttentionSpanTestState['step']) => void;
  reset: () => void;
}
...
export const useAttentionSpanTestStore = create<AttentionSpanTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: AttentionSpanQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: AttentionSpanAnswer) => {
      ...
      setStep: (step: AttentionSpanTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'attention-span-test-storage',
      partialize: (state) => ({
