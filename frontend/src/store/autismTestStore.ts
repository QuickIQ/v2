...
export interface AutismAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface AutismTestState {
  // Session
  ...
  setQuestions: (questions: AutismQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: AutismAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: AutismTestState['step']) => void;
  reset: () => void;
}
...
export const useAutismTestStore = create<AutismTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: AutismQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: AutismAnswer) => {
      ...
      setStep: (step: AutismTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'autism-test-storage',
      partialize: (state) => ({
