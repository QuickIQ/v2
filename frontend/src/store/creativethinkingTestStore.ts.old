...
export interface CreativeThinkingQuestion {
  id: number;
  text: string;
...
export interface CreativeThinkingAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface CreativeThinkingTestState {
  // Session
  ...
  answers: CreativeThinkingAnswer[];
  ...
  setQuestions: (questions: CreativeThinkingQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: CreativeThinkingAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: CreativeThinkingTestState['step']) => void;
  reset: () => void;
}
...
export const useCreativeThinkingTestStore = create<CreativeThinkingTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: CreativeThinkingQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: CreativeThinkingAnswer) => {
      ...
      setStep: (step: CreativeThinkingTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'creative-thinking-test-storage',
      partialize: (state) => ({
