...
export interface MultitaskingQuestion {
  id: number;
  text: string;
...
export interface MultitaskingAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface MultitaskingTestState {
  // Session
  ...
  setQuestions: (questions: MultitaskingQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: MultitaskingAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: MultitaskingTestState['step']) => void;
  reset: () => void;
}
...
export const useMultitaskingTestStore = create<MultitaskingTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: MultitaskingQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: MultitaskingAnswer) => {
      ...
      setStep: (step: MultitaskingTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'multitasking-test-storage',
      partialize: (state) => ({
