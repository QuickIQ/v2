...
export interface TimeManagementAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface TimeManagementTestState {
  // Session
  ...
  setQuestions: (questions: TimeManagementQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: TimeManagementAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: TimeManagementTestState['step']) => void;
  reset: () => void;
}
...
export const useTimeManagementTestStore = create<TimeManagementTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: TimeManagementQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: TimeManagementAnswer) => {
      ...
      setStep: (step: TimeManagementTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'time-management-test-storage',
      partialize: (state) => ({
