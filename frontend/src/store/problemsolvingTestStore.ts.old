...
export interface ProblemSolvingAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface ProblemSolvingTestState {
  // Session
  ...
  setQuestions: (questions: ProblemSolvingQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: ProblemSolvingAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: ProblemSolvingTestState['step']) => void;
  reset: () => void;
}
...
export const useProblemSolvingTestStore = create<ProblemSolvingTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: ProblemSolvingQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: ProblemSolvingAnswer) => {
      ...
      setStep: (step: ProblemSolvingTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'problem-solving-test-storage',
      partialize: (state) => ({
