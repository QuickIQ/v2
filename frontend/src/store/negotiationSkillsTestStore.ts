...
export interface NegotiationSkillsQuestion {
  id: number;
  text: string;
...
export interface NegotiationSkillsAnswer {
  question_id: number;
  option_index: number;
  score: number; // 1-7 based on option index
}

export interface NegotiationSkillsTestState {
  // Session
  ...
  answers: NegotiationSkillsAnswer[];
  ...
  setQuestions: (questions: NegotiationSkillsQuestion[]) => void;
  setCurrentQuestionIndex: (index: number) => void;
  addAnswer: (answer: NegotiationSkillsAnswer) => void;
  calculateScore: () => void;
  setResultData: (data: any) => void;
  setEmail: (email: string) => void;
  setStep: (step: NegotiationSkillsTestState['step']) => void;
  reset: () => void;
}
...
export const useNegotiationSkillsTestStore = create<NegotiationSkillsTestState>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQuestions: (questions: NegotiationSkillsQuestion[]) => set({ questions }),
      
      setCurrentQuestionIndex: (index: number) => set({ currentQuestionIndex: index }),
      
      addAnswer: (answer: NegotiationSkillsAnswer) => {
      ...
      setStep: (step: NegotiationSkillsTestState['step']) => set({ step }),
      
      reset: () => set(initialState),
    }),
    {
      name: 'negotiation-skills-test-storage',
      partialize: (state) => ({
