...
import TeamPlayerIntro from '../tests/iq/team-player/index';
import TeamPlayerQuestionsPage from '../tests/iq/team-player/QuestionsPage';
import TeamPlayerAnalyzingPage from '../tests/iq/team-player/TeamPlayerAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/team-player/resultContent';
import questionsData from '../data/tests/team-player/questions.json';

function TeamPlayerTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useTeamPlayerTestStore.getState();
    ...
    try {
      const storeQuestions = useTeamPlayerTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useTeamPlayerTestStore.setState({ answers: [] });
    useTeamPlayerTestStore.setState({ totalScore: 0 });
    useTeamPlayerTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useTeamPlayerTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useTeamPlayerTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/team-player/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.teamPlayer.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <TeamPlayerIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useTeamPlayerTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <TeamPlayerAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default TeamPlayerTestPage;
