...
import StrategicThinkingIntro from '../tests/iq/strategic-thinking/index';
import StrategicThinkingQuestionsPage from '../tests/iq/strategic-thinking/QuestionsPage';
import StrategicThinkingAnalyzingPage from '../tests/iq/strategic-thinking/StrategicThinkingAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/strategic-thinking/resultContent';
import questionsData from '../data/tests/strategic-thinking/questions.json';

function StrategicThinkingTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useStrategicThinkingTestStore.getState();
    ...
    try {
      const storeQuestions = useStrategicThinkingTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useStrategicThinkingTestStore.setState({ answers: [] });
    useStrategicThinkingTestStore.setState({ totalScore: 0 });
    useStrategicThinkingTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useStrategicThinkingTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useStrategicThinkingTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/strategic-thinking/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.strategicThinking.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <StrategicThinkingIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useStrategicThinkingTestStore.getState().questions;
      ...
      if (questionsToUse.length === 0) {
        ...
        return (
          <div style={{
            ...
            <div className="loading" style={{ fontSize: '18px', color: '#4A6CFF' }}>
              {t('common.loading')}
            </div>
          </div>
        );
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <StrategicThinkingQuestionsPage
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <StrategicThinkingAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default StrategicThinkingTestPage;
