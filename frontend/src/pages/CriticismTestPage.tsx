...
import CriticismIntro from '../tests/iq/criticism/index';
import CriticismQuestionsPage from '../tests/iq/criticism/QuestionsPage';
import CriticismAnalyzingPage from '../tests/iq/criticism/CriticismAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/criticism/resultContent';
import questionsData from '../data/tests/criticism/questions.json';

function CriticismTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useCriticismTestStore.getState();
    ...
    try {
      const storeQuestions = useCriticismTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useCriticismTestStore.setState({ answers: [] });
    useCriticismTestStore.setState({ totalScore: 0 });
    useCriticismTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useCriticismTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useCriticismTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/criticism/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.criticism.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <CriticismIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useCriticismTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <CriticismAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default CriticismTestPage;
