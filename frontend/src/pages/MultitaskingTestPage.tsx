...
import MultitaskingIntro from '../tests/iq/multitasking/index';
import MultitaskingQuestionsPage from '../tests/iq/multitasking/QuestionsPage';
import MultitaskingAnalyzingPage from '../tests/iq/multitasking/MultitaskingAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/multitasking/resultContent';
import questionsData from '../data/tests/multitasking/questions.json';

function MultitaskingTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useMultitaskingTestStore.getState();
    ...
    try {
      const storeQuestions = useMultitaskingTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useMultitaskingTestStore.setState({ answers: [] });
    useMultitaskingTestStore.setState({ totalScore: 0 });
    useMultitaskingTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useMultitaskingTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      let store = useMultitaskingTestStore.getState();
      ...
      const storeInstance = useMultitaskingTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/multitasking/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.multitasking.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (error && step !== 'landing') {
    return (
      <div style={{
        ...
        background: 'linear-gradient(135deg, #E4ECFF 0%, #F0E8FF 100%)',
        padding: '40px',
      }}>
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <MultitaskingIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useMultitaskingTestStore.getState().questions;
      ...
      if (questionsToUse.length === 0) {
        ...
        return (
          <div style={{
            ...
            <div className="loading" style={{ fontSize: '18px', color: '#6366F1' }}>
              {t('common.loading')}
            </div>
          </div>
        );
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <MultitaskingAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default MultitaskingTestPage;
