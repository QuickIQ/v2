...
import PerfectionismIntro from '../tests/iq/perfectionism/index';
import PerfectionismQuestionsPage from '../tests/iq/perfectionism/QuestionsPage';
import PerfectionismAnalyzingPage from '../tests/iq/perfectionism/PerfectionismAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/perfectionism/resultContent';
import questionsData from '../data/tests/perfectionism/questions.json';
...
function PerfectionismTestPage() {
  ...
  useEffect(() => {
    ...
    const store = usePerfectionismTestStore.getState();
    ...
    try {
      const storeQuestions = usePerfectionismTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    usePerfectionismTestStore.setState({ answers: [] });
    usePerfectionismTestStore.setState({ totalScore: 0 });
    usePerfectionismTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = usePerfectionismTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = usePerfectionismTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/perfectionism/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.perfectionism.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <PerfectionismIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = usePerfectionismTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <PerfectionismAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
