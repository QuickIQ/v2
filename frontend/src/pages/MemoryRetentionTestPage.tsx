...
import MemoryRetentionIntro from '../tests/iq/memory-retention/index';
import MemoryRetentionQuestionsPage from '../tests/iq/memory-retention/QuestionsPage';
import MemoryRetentionAnalyzingPage from '../tests/iq/memory-retention/MemoryRetentionAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/memory-retention/resultContent';
import questionsData from '../data/tests/memory-retention/questions.json';
...
function MemoryRetentionTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useMemoryRetentionTestStore.getState();
    ...
    try {
      const storeQuestions = useMemoryRetentionTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    const storeQuestions = useMemoryRetentionTestStore.getState().questions;
    console.log('📋 Store questions:', storeQuestions.length);
    ...
    useMemoryRetentionTestStore.setState({ answers: [] });
    useMemoryRetentionTestStore.setState({ totalScore: 0 });
    useMemoryRetentionTestStore.setState({ resultLevel: null });
    
    console.log('➡️ Setting step to questions...');
    setStep('questions');
    setPhase('questions');
    
    // Verify step was set
    setTimeout(() => {
      const storeAfterSet = useMemoryRetentionTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useMemoryRetentionTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/memory-retention/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.memoryRetention.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (!isInitialized) {
    return (
      <div style={{
        ...
        <div className="loading" style={{ fontSize: '18px', color: '#9C27B0' }}>
          {t('common.loading')}
        </div>
      </div>
    );
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <MemoryRetentionIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useMemoryRetentionTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <MemoryRetentionAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
