...
import AnxietyIntro from '../tests/iq/anxiety/index';
import AnxietyQuestionsPage from '../tests/iq/anxiety/QuestionsPage';
import AnxietyAnalyzingPage from '../tests/iq/anxiety/AnxietyAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/anxiety/resultContent';
import questionsData from '../data/tests/anxiety/questions.json';
...
function AnxietyTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useAnxietyTestStore.getState();
    ...
    try {
      const storeQuestions = useAnxietyTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useAnxietyTestStore.setState({ answers: [] });
    useAnxietyTestStore.setState({ totalScore: 0 });
    useAnxietyTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useAnxietyTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useAnxietyTestStore.getState();
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.anxiety.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (!isInitialized) {
    return (
      <div style={{
        ...
        <div className="loading" style={{ fontSize: '18px', color: '#FF9800' }}>
          {t('common.loading')}
        </div>
      </div>
    );
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <AnxietyIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useAnxietyTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <AnxietyAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
