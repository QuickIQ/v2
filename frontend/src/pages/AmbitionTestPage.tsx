...
import AmbitionIntro from '../tests/iq/ambition/index';
import AmbitionQuestionsPage from '../tests/iq/ambition/QuestionsPage';
import AmbitionAnalyzingPage from '../tests/iq/ambition/AmbitionAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/ambition/resultContent';
import questionsData from '../data/tests/ambition/questions.json';
...
function AmbitionTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useAmbitionTestStore.getState();
    ...
    try {
      const storeQuestions = useAmbitionTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useAmbitionTestStore.setState({ answers: [] });
    useAmbitionTestStore.setState({ totalScore: 0 });
    useAmbitionTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useAmbitionTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useAmbitionTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/ambition/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.ambition.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <AmbitionIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useAmbitionTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <AmbitionAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
