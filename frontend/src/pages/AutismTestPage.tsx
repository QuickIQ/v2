...
import AutismIntro from '../tests/iq/autism/index';
import AutismQuestionsPage from '../tests/iq/autism/QuestionsPage';
import AutismAnalyzingPage from '../tests/iq/autism/AutismAnalyzingPage';
...
function AutismTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useAutismTestStore.getState();
    ...
    try {
      const storeQuestions = useAutismTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useAutismTestStore.setState({ answers: [] });
    useAutismTestStore.setState({ totalScore: 0 });
    useAutismTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useAutismTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useAutismTestStore.getState();
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.autism.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <AutismIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useAutismTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <AutismAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
