...
import NegotiationSkillsIntro from '../tests/iq/negotiation-skills/index';
import NegotiationSkillsQuestionsPage from '../tests/iq/negotiation-skills/QuestionsPage';
import NegotiationSkillsAnalyzingPage from '../tests/iq/negotiation-skills/NegotiationSkillsAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/negotiation-skills/resultContent';
import questionsData from '../data/tests/negotiation-skills/questions.json';

function NegotiationSkillsTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useNegotiationSkillsTestStore.getState();
    ...
    try {
      const storeQuestions = useNegotiationSkillsTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useNegotiationSkillsTestStore.setState({ answers: [] });
    useNegotiationSkillsTestStore.setState({ totalScore: 0 });
    useNegotiationSkillsTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useNegotiationSkillsTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useNegotiationSkillsTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/negotiation-skills/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.negotiationSkills.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <NegotiationSkillsIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useNegotiationSkillsTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <NegotiationSkillsQuestionsPage
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <NegotiationSkillsAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default NegotiationSkillsTestPage;
