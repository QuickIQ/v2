...
import TimeManagementIntro from '../tests/iq/time-management/index';
import TimeManagementQuestionsPage from '../tests/iq/time-management/QuestionsPage';
import TimeManagementAnalyzingPage from '../tests/iq/time-management/TimeManagementAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/time-management/resultContent';
import questionsData from '../data/tests/time-management/questions.json';
...
function TimeManagementTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useTimeManagementTestStore.getState();
    ...
    try {
      const storeQuestions = useTimeManagementTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useTimeManagementTestStore.setState({ answers: [] });
    useTimeManagementTestStore.setState({ totalScore: 0 });
    useTimeManagementTestStore.setState({ resultLevel: null });
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    // Navigate to payment page
      navigate('/test/time-management/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.timeManagement.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <TimeManagementIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useTimeManagementTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <TimeManagementAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
