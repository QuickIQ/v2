...
import CreativeThinkingIntro from '../tests/iq/creative-thinking/index';
import CreativeThinkingQuestionsPage from '../tests/iq/creative-thinking/QuestionsPage';
import CreativeThinkingAnalyzingPage from '../tests/iq/creative-thinking/CreativeThinkingAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/creative-thinking/resultContent';
import questionsData from '../data/tests/creative-thinking/questions.json';
...
function CreativeThinkingTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useCreativeThinkingTestStore.getState();
    ...
    try {
      const storeQuestions = useCreativeThinkingTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useCreativeThinkingTestStore.setState({ answers: [] });
    useCreativeThinkingTestStore.setState({ totalScore: 0 });
    useCreativeThinkingTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useCreativeThinkingTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useCreativeThinkingTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/creative-thinking/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.creativeThinking.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <CreativeThinkingIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useCreativeThinkingTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <CreativeThinkingAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
