...
import AttentionSpanIntro from '../tests/iq/attention-span/index';
import AttentionSpanQuestionsPage from '../tests/iq/attention-span/QuestionsPage';
import AttentionSpanAnalyzingPage from '../tests/iq/attention-span/AttentionSpanAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/attention-span/resultContent';
import questionsData from '../data/tests/attention-span/questions.json';
...
function AttentionSpanTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useAttentionSpanTestStore.getState();
    ...
    try {
      const storeQuestions = useAttentionSpanTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useAttentionSpanTestStore.setState({ answers: [] });
    useAttentionSpanTestStore.setState({ totalScore: 0 });
    useAttentionSpanTestStore.setState({ resultLevel: null });
    
    // Update both component state and store state
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useAttentionSpanTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useAttentionSpanTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/attention-span/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.attentionSpan.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (!isInitialized) {
    return (
      <div style={{
        ...
        <div className="loading" style={{ fontSize: '18px', color: '#4CAF50' }}>
          {t('common.loading')}
        </div>
      </div>
    );
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <AttentionSpanIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useAttentionSpanTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <AttentionSpanAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
