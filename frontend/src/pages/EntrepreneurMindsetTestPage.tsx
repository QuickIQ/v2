...
import EntrepreneurMindsetIntro from '../tests/iq/entrepreneur-mindset/index';
import EntrepreneurMindsetQuestionsPage from '../tests/iq/entrepreneur-mindset/QuestionsPage';
import EntrepreneurMindsetAnalyzingPage from '../tests/iq/entrepreneur-mindset/EntrepreneurMindsetAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/entrepreneur-mindset/resultContent';
import questionsData from '../data/tests/entrepreneur-mindset/questions.json';
...
function EntrepreneurMindsetTestPage() {
  ...
  const {
    ...
    setResultData,
    setEmail,
  } = useEntrepreneurMindsetTestStore();
  ...
  useEffect(() => {
    ...
    const store = useEntrepreneurMindsetTestStore.getState();
    ...
    try {
      const storeQuestions = useEntrepreneurMindsetTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useEntrepreneurMindsetTestStore.setState({ answers: [] });
    useEntrepreneurMindsetTestStore.setState({ totalScore: 0 });
    useEntrepreneurMindsetTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useEntrepreneurMindsetTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useEntrepreneurMindsetTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    // Navigate to payment page
      navigate('/test/entrepreneur-mindset/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.entrepreneurMindset.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (error && step !== 'landing') {
    return (
      <div style={{
        ...
        background: 'linear-gradient(135deg, #E8F0FF 0%, #FFF9E6 100%)',
        padding: '40px',
      }}>
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <EntrepreneurMindsetIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useEntrepreneurMindsetTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <EntrepreneurMindsetQuestionsPage
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <EntrepreneurMindsetAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
