...
import DecisionMakingIntro from '../tests/iq/decision-making/index';
import DecisionMakingQuestionsPage from '../tests/iq/decision-making/QuestionsPage';
import DecisionMakingAnalyzingPage from '../tests/iq/decision-making/DecisionMakingAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/decision-making/resultContent';
import questionsData from '../data/tests/decision-making/questions.json';

function DecisionMakingTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useDecisionMakingTestStore.getState();
    ...
    try {
      const storeQuestions = useDecisionMakingTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useDecisionMakingTestStore.setState({ answers: [] });
    useDecisionMakingTestStore.setState({ totalScore: 0 });
    useDecisionMakingTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useDecisionMakingTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useDecisionMakingTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/decision-making/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.decisionMaking.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <DecisionMakingIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useDecisionMakingTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <DecisionMakingAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
...
export default DecisionMakingTestPage;
