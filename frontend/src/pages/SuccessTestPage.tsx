...
import SuccessIntro from '../tests/iq/success/index';
import SuccessQuestionsPage from '../tests/iq/success/QuestionsPage';
import SuccessAnalyzingPage from '../tests/iq/success/SuccessAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/success/resultContent';
import questionsData from '../data/tests/success/questions.json';

function SuccessTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useSuccessTestStore.getState();
    ...
    try {
      const storeQuestions = useSuccessTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useSuccessTestStore.setState({ answers: [] });
    useSuccessTestStore.setState({ totalScore: 0 });
    useSuccessTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useSuccessTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useSuccessTestStore.getState();
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.success.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <SuccessIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useSuccessTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <SuccessAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
