...
import RiskToleranceIntro from '../tests/iq/risk-tolerance/index';
import RiskToleranceQuestionsPage from '../tests/iq/risk-tolerance/QuestionsPage';
import RiskToleranceAnalyzingPage from '../tests/iq/risk-tolerance/RiskToleranceAnalyzingPage';
import PersonalityEmailPage from '../tests/personality/PersonalityEmailPage';
import { resultContent } from '../tests/iq/risk-tolerance/resultContent';
import questionsData from '../data/tests/risk-tolerance/questions.json';
...
function RiskToleranceTestPage() {
  ...
  useEffect(() => {
    ...
    const store = useRiskToleranceTestStore.getState();
    ...
    try {
      const storeQuestions = useRiskToleranceTestStore.getState().questions;
  ...
  const handleStart = () => {
    ...
    useRiskToleranceTestStore.setState({ answers: [] });
    useRiskToleranceTestStore.setState({ totalScore: 0 });
    useRiskToleranceTestStore.setState({ resultLevel: null });
  ...
  const handleQuestionsComplete = async () => {
    ...
    try {
      ...
      // Verify step was set
      const storeAfterSet = useRiskToleranceTestStore.getState();
  ...
  const handleAnalyzingComplete = async () => {
    try {
      ...
      const storeInstance = useRiskToleranceTestStore.getState();
  ...
  const handleEmailSubmit = async (emailValue: string, acceptedTerms: boolean, acceptedPrivacy: boolean) => {
    ...
    navigate('/test/risk-tolerance/payment');
  };
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (store.step === 'questions' && store.answers.length > 0) {
        e.preventDefault();
        e.returnValue = t('tests.riskTolerance.warnings.leave_page') || 'If you leave this page, your current test progress will be lost. Continue?';
  ...
  if (step === 'landing' || phase === 'intro') {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          <RiskToleranceIntro onStart={handleStart} />
        </motion.div>
      </AnimatePresence>
    );
  }
  ...
  switch (step) {
    case 'questions':
      const currentQuestionsFromStore = useRiskToleranceTestStore.getState().questions;
      ...
      return (
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.4 }}
          >
            <RiskToleranceAnalyzingPage onComplete={handleAnalyzingComplete} />
          </motion.div>
        </AnimatePresence>
      );
    case 'email':
