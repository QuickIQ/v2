...
function PersonalityAnalyzingPage({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const hydrationTimeout = setTimeout(() => {
      ...
      console.log('  - Current step:', store.step);
      
      // Check if we have valid answers
      if (storeAnswers.length === 0 || storeQuestions.length === 0) {
        console.warn('⚠️ No answers or questions found, waiting longer for hydration...');
        
        // Wait longer (500ms) before checking again to allow for async hydration
        redirectTimeoutRef.current = setTimeout(() => {
          const finalCheck = usePersonalityTestStore.getState();
          console.log('🔍 Final hydration check:');
          console.log('  - Answers:', finalCheck.answers.length);
          console.log('  - Questions:', finalCheck.questions.length);
          console.log('  - Step:', finalCheck.step);
          
          // Only fail if we're still on analyzing step and still have no answers
          // If step changed, don't interfere
          if (finalCheck.step === 'analyzing') {
            if (finalCheck.answers.length === 0 || finalCheck.questions.length === 0) {
              console.error('❌ Still no answers after hydration delay. This should not happen if user completed questions.');
            ...
            // Step changed, don't interfere
            console.log('ℹ️ Step changed to:', finalCheck.step, '- not interfering');
            setIsHydrated(true);
          }
    ...
    }, 150); // Increased initial delay to 150ms
  ...
  // Check if we have data even if hydration flag isn't set
  // This prevents false redirects when data exists but hydration is delayed
  useEffect(() => {
    const store = usePersonalityTestStore.getState();
    const hasAnswers = (answers && answers.length > 0) || (store.answers && store.answers.length > 0);
    const hasQuestions = (questions && questions.length > 0) || (store.questions && store.questions.length > 0);
    
    // If we have data but hydration flag isn't set, set it now
    if (!isHydrated && hasAnswers && hasQuestions) {
      console.log('✅ Have answers/questions, setting hydration flag');
      setIsHydrated(true);
    }
  }, [answers, questions, isHydrated]);

  // Show loading if not hydrated yet, but check if we actually have data
  const store = usePersonalityTestStore.getState();
  const hasAnswers = (answers && answers.length > 0) || (store.answers && store.answers.length > 0);
  const hasQuestions = (questions && questions.length > 0) || (store.questions && store.questions.length > 0);
  
  // Only show loading if we truly don't have data AND hydration hasn't completed
  // If we have data, proceed even if hydration flag isn't set yet
  if (!isHydrated && (!hasAnswers || !hasQuestions)) {
